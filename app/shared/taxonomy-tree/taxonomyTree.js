    ...
    return {
      ...
      link: function (scope) {
        ...
        scope.getAndSetParents = function (node, preserve) {
          ...
          snowowlService.getConceptParents(null, null, node.conceptId, scope.branch).then(function (parents) {
              scope.array = [];
              if (!parents) {
                console.error('Could not retrieve children for node', node);
                deferred.resolve([]);
        ...
        scope.constructRootTreesHelper = function (node) {
          ...
          if (scope.concept) {
            ...
            if(scope.view){
                ...
                if (scope.concept.hasOwnProperty('isLeafInferred')) {
                  scope.constructRootTrees(null, null,scope.concept);
                }
                ...
                // otherwise retrieve the full concept to ensure all required information is available (search sometimes fails to return laf status)
                  ...
                  snowowlService.getFullConcept(null, null, scope.concept.conceptId, scope.branch).then(function(response) {
...
//          }
        ...
        scope.$on('reloadTaxonomy', function() {
            initialize();
        });
