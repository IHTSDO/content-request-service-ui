    ...
    .controller('RequestListCtrl', [
        ...
        function ($filter, $sce, crsJiraService, NgTableParams, requestService, notificationService, accountService, jiraService, $routeParams, $uibModal, utilsService, $scope, scaService, BULK_ACTION_STATUS, BULK_ACTION, DEFAULT_COLUMNS, $timeout, STATISTICS_STATUS, configService) {
            ...
            var initView = function () {
                ...
                //load topic options
                vm.topicOptions = angular.copy(requestService.getTopics());
                if(!vm.topicOptions){
                    loadTopicOptions();
                }
            ...
            var loadAuthors = function () {
                ...
                return crsJiraService.getAuthorUsers(0, 50, true, [], groupName).then(function (users) {
                    ...
                    for(var i in vm.authors){
                        ...
                        vm.authors[i].id = vm.authors[i].name;
                    }
            ...
            var loadStaff = function() {
                ...
                return crsJiraService.getAuthorUsers(0, 50, true, [], groupName).then(function(users) {
                    ...
                    for(var i in vm.staffs){
                        ...
                        vm.staffs[i].id = vm.staffs[i].name;
                    }
            ...
            var loadRequestors = function() {
                ...
                return crsJiraService.getAuthorUsers(0, 50, true, [], groupName).then(function(users) {
                    ...
                    for(var i in vm.requestors){
                        ...
                        vm.requestors[i].id = vm.requestors[i].name;
                    }
            ...
            var loadTopicOptions = function(){
                return crsJiraService.getTopicOptions().then(function(topicOptions){                    
                    ...
                    requestService.setTopics(topicOptions);
                    vm.topicOptions = angular.copy(topicOptions);
                });
            };
            ...
            $scope.$watch(function() {
                return vm.selectedTopics;
            }, function(newVal, oldVal) {              
                if(vm.requests &&  JSON.stringify(oldVal) !== JSON.stringify(newVal)){
            ...
            var openAssignRequestModal = function(selectedRequestIds) {
                ...
                modalInstance.result.then(function(rs) {
                    ...
                    var assigningData = {
                        data: {
                            assignee: rs.assignee? rs.assignee.name : null,
            ...
            var openAssignRequestToStaffModal = function(selectedRequestIds) {
                ...
                modalInstance.result.then(function(rs) {
                    ...
                    var data = {
                        data: {
                            manager: rs.assignee? rs.assignee.name : null
                        },
                        requestIds: selectedRequestIds
                    };
                    requestService.bulkAction(data, action).then(function (response) {
            ...
            var openReassignRequestsToRequestorModal = function(selectedRequestIds) {
                ...
                modalInstance.result.then(function(rs) {
                    ...
                    var data = {
                        data: {
                            reporter: rs.assignee? rs.assignee.name : null
                        },
                        requestIds: selectedRequestIds
                    };
                    requestService.bulkAction(data, action).then(function (response) {
            ...
            var buildRequestList = function (typeList, page, pageCount, search, sortFields, sortDirs, batchRequest, fsn, jiraTicketId, requestDateFrom, requestDateTo, topic, summary, trackerId, manager, status, author, requestId, requestType, showClosedRequests, statusDateFrom, statusDateTo, lastStatusModifier, forwardDestinationId, memberCountry){
                ...
                requestList.organization = memberCountry;                
            ...
            var requestTableParams = new NgTableParams({
                ...
                },
                {
                    ...
                    getData: function (params) {
                        ...
                        var myRequests;
                        var filterValues;
                        var defaultSorting = {'requestHeader.statusDate': 'desc', id: 'desc'};
                        vm.showResetTableSorting = JSON.stringify(defaultSorting) !== JSON.stringify(sortingObj);
                        if (sortingObj) {
                        ...
                        if (vm.selectedTopics.length !== 0) {
                            angular.forEach(vm.selectedTopics, function(item) {
                                expectedTopics = expectedTopics.concat(Object.values(item));
                            });
                        }
            ...
            var submittedTableParams = new NgTableParams({
                ...
                
                {
                    ...
                    getData: function (params) {
                        ...
                        var filterValues;
                        var subbmitedRequests;
                        var defaultSorting = {'requestHeader.requestDate': 'desc', batchRequest: 'desc', id: 'desc'};
                        vm.showResetTableSorting = JSON.stringify(defaultSorting) !== JSON.stringify(sortingObj);
            ...
            var resetTableSorting = function(tableParams, defaultSorting) {
                tableParams.sorting(defaultSorting);
                tableParams.reload();
            };
            ...
            vm.resetTableSorting = resetTableSorting;
            vm.showResetTableSorting = false;
            vm.daterange = {
                startDate: null,
                endDate: null
            };
            ...
            vm.multiSelectTranslation = {                
                ...
                reset           : "Undo all",
                search          : "Search...",
                nothingSelected : "Topics...",
                selectedItems   : "Selected Topics" 
            };
