...
angular
    ...
    .controller('RequestDetailsCtrl', [
        ...
        function ($scope, $rootScope, $routeParams, $location, $uibModal, $sce, $q, requestService, notificationService, requestMetadataService, objectService, snowowlService, snowowlMetadataService, crsJiraService, scaService, accountService, REQUEST_METADATA_KEY, REQUEST_TYPE, CONCEPT_EDIT_EVENT, REQUEST_STATUS, REQUEST_INPUT_MODE, jiraService, $timeout, utilsService, $filter, STATISTICS_STATUS, configService) {
            ...
            var getAuthorName = function(authorName) {
                var usersList = [];
                ...
                if (!usersList || usersList.length === 0) {
                    ...
                    for (var i = 0; i < usersList.length; i++) {
                        // if (vm.authors[i].name !== authorName) {
                        //     return authorName;
                        // }
                        if (usersList[i].name === authorName) {
            ...
            var getStaffName = function(staffName) {
                if (!vm.staffs || vm.staffs.length === 0) {
                    return staffName;
                } else {
                    for (var i = 0; i < vm.staffs.length; i++) {
                        if (vm.staffs[i].name === staffName) {
                            //return vm.authors[i].displayName;
                            return $sce.trustAsHtml([
            ...
            var buildRequestFromRequestData = function(requestData) {
                var request = {
                    ...
                    forwardDestinationBrowserUrl: (requestData.forwardDestinationBrowserUrl ? requestData.forwardDestinationBrowserUrl.replace('/preview', '/edit') : '') + '/' + requestData.forwardDestinationId,
                    forwardSourceId: requestData.forwardSourceId,
                    forwardSourceBrowserUrl: requestData.forwardSourceBrowserUrl + '/' + requestData.forwardSourceId,
            ...
            var saveAndSubmitRequest = function() {
                ...
                if (vm.inputMode !== REQUEST_INPUT_MODE.DIRECT) {
                    if (vm.requestType === REQUEST_TYPE.RETIRE_DESCRIPTION || vm.requestType === REQUEST_TYPE.RETIRE_RELATIONSHIP || vm.requestType === REQUEST_TYPE.CHANGE_DESCRIPTION || vm.requestType === REQUEST_TYPE.CHANGE_RELATIONSHIP || vm.requestType === REQUEST_TYPE.NEW_RELATIONSHIP || vm.requestType === REQUEST_TYPE.NEW_DESCRIPTION) {
                        var j = requestData.requestItems.length;
                        while (j--) {
                            if (requestData.requestItems[j] && requestData.requestItems[j].requestType === REQUEST_TYPE.CHANGE_RETIRE_CONCEPT.value) {
                                requestData.requestItems.splice(j, 1);
                            }
                            if (requestData.requestItems[j] && requestData.requestItems[j].requestType === REQUEST_TYPE.NEW_RELATIONSHIP.value && vm.requestType !== REQUEST_TYPE.NEW_RELATIONSHIP) {
                                requestData.requestItems.splice(j, 1);
                            }
                            if(requestData.requestItems[j] && vm.requestType === REQUEST_TYPE.RETIRE_DESCRIPTION){
                                if (requestData.requestItems[j].requestType === REQUEST_TYPE.NEW_DESCRIPTION.value) {
                                    requestData.requestItems.splice(j, 1);
                                }
                            } 
            ...
            var assignRequest = function() {
                if (vm.authors.length > 0 && vm.projects.length > 0) {
                    ...
                    modalInstance.result.then(function(rs) {
                        ...
                        requestService.assignRequests([vm.request.id], rs.project.key, ((rs.assignee) ? rs.assignee.name : null), rs.summary).then(function() {
            ...
            var acceptAndAssignRequest = function() {
                if (vm.authors.length > 0 && vm.projects.length > 0) {
                    ...
                    modalInstance.result.then(function(rs) {
                        changeRequestStatus(vm.request.id, REQUEST_STATUS.ACCEPTED)
                            .then(function() {
                                return requestService.assignRequests([vm.request.id], rs.project.key, ((rs.assignee) ? rs.assignee.name : null), rs.summary);
                            }, function(e) {
                                showErrorMessage(e.message);
                                $q.reject(e);
                            })
                ...
                if (vm.staffs.length > 0) {
                    ...
                    modalInstance.result.then(function(rs) {
                        if(status === REQUEST_STATUS.NEW.value){
                            changeRequestStatus(vm.request.id, REQUEST_STATUS.ACCEPTED)
                            .then(function() {
                                return requestService.assignRequestsToStaff([vm.request.id], ((rs.assignee) ? rs.assignee.name : null));
                            }, function(e) {
                                showErrorMessage(e.message);
                                $q.reject(e);
                            })
                            ...
                            notificationService.sendMessage('Assigning requests');
                            requestService.assignRequestsToStaff([vm.request.id], ((rs.assignee) ? rs.assignee.name : null)).then(function() {
                                notificationService.sendMessage('Request assigned successfully', 5000);
                                goBackToPreviousList();
                            });
                        }
            ...
            var reassignToRequestor = function() {
                if (vm.staffs.length > 0) {
                    ...
                    modalInstance.result.then(function(rs) {
                            ...
                            return requestService.reassignRequestToRequestor([vm.request.id], ((rs.assignee) ? rs.assignee.name : null))
                            .then(function() {
            ...
            var setInputMode = function(im) {
                ...
                if (imObj !== null && imObj !== vm.inputMode) {
                    ...
                    vm.inputMode = imObj;
                    $routeParams.inputMode = vm.inputMode;
                }
            };
